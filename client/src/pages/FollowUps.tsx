...
export default function FollowUps() {
  ...
  const now = new Date();
  const overdue = items.filter((f) => toDate(f.scheduled_at) < now);
  const upcoming = items.filter((f) => toDate(f.scheduled_at) >= now);

  // group upcoming by date
  const groups: Record<string, FollowUp[]> = {};
  upcoming.forEach((f) => {
    const key = format(toDate(f.scheduled_at), 'EEEE, MMM d');
